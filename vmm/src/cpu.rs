...
impl CpuManager {
    ...
    ///   - Each vCPU runs for up to SLICE_EXITS real VM exits per turn.
    ///   - A preemption timer thread sends SIGRTMIN to force KVM_RUN to return
    ///     EINTR (→ VmExit::Ignore) if a vCPU blocks in-kernel too long.  This
    ///     handles the BSP spinning during SMP bringup without generating exits.
    ...
    fn start_det_scheduler(&mut self, desired_vcpus: u32) -> Result<()> {
        ...
        info!("[det-sched] Starting deterministic scheduler with {} vCPU(s)", vcpu_count);

        // Collect per-vCPU handles and state.
        let vcpus: Vec<Arc<Mutex<Vcpu>>> = (0..vcpu_count)
            .map(|i| Arc::clone(&self.vcpus[i]))
            .collect();

        // Shared signals — same ones that start_vcpu wires up.
        let vcpus_kill_signalled   = self.vcpus_kill_signalled.clone();
        let vcpus_pause_signalled  = self.vcpus_pause_signalled.clone();
        let exit_evt               = self.exit_evt.try_clone().unwrap();
        let reset_evt              = self.reset_evt.try_clone().unwrap();
        ...
        // Per-vCPU kill / paused / interrupted flags — need one set per vCPU
        // so that the existing pause/resume/shutdown machinery still works.
        let mut vcpu_states = self.vcpu_states.lock().unwrap();
        ...
        // Preemption timer: a lightweight thread that sends SIGRTMIN to the
        // scheduler thread every PREEMPT_US microseconds.  This forces KVM_RUN
        // to return EINTR -> VmExit::Ignore when the vCPU blocks in-kernel
        // without generating userspace-visible exits (e.g., BSP spinning during
        // SMP bringup).  The signal is harmless: the existing signal handler
        // does nothing; the interrupted KVM_RUN just restarts from our loop.
        //
        // 1 ms gives the guest enough time to make progress per slice while
        // still switching vCPUs frequently enough for SMP bringup to work.
        const PREEMPT_US: u64 = 1_000; // 1 ms
            ...
            thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    loop {
                        ...
                        if tid != 0 {
                            // SAFETY: tid is valid as long as the scheduler
                            // thread is alive (guaranteed by preempt_stop).
                            unsafe {
                ...
                .expect("failed to spawn det-preempt thread");
        }

        let handle = thread::Builder::new()
            .name("det-sched".to_string())
            .spawn(move || {
                // Publish our pthread_t so the preemption timer can reach us.
                // pthread_self() returns the handle for the current thread.
                // SAFETY: always safe.
                let my_pt = unsafe { libc::pthread_self() } as u64;
                sched_tid.store(my_pt, Ordering::Relaxed);

                // Register the SIGRTMIN handler so that the preemption
                // timer's pthread_kill() is handled gracefully (interrupts
                // KVM_RUN with EINTR rather than killing the process).
                ...
                if !vcpu_seccomp_filter.is_empty() {
                    if let Err(e) = apply_filter(&vcpu_seccomp_filter)
                        .map_err(Error::ApplySeccompFilter)
                    {
                        error!("[det-sched] Failed to apply seccomp filter: {e:?}");
                        exit_evt.write(1).unwrap();
                        preempt_stop.store(true, Ordering::Relaxed);
                        return;
                    }
                }
                ...
                // Number of "real" (non-Ignore) VM exits each vCPU runs per
                // scheduling round before we switch to the next vCPU.
                // Ignore exits (HLT wake, in-kernel handling, etc.) are free
                // and don't consume the quota — otherwise a halted AP would
                // starve the BSP by burning the entire round on Ignore exits.
                ...
                'outer: loop {
                    ...
                    // --- Kill check ---
                    if vcpus_kill_signalled.load(Ordering::SeqCst) {
                        for f in &interrupted_flags { f.store(true, Ordering::SeqCst); }
                        break 'outer;
                    }
                    ...
                    // --- Run each vCPU for SLICE_EXITS real exits ---
                    for &vcpu_idx in &order {
                        ...
                        let mut real_exits: u32 = 0;
                        while real_exits < SLICE_EXITS {
                            ...
                            let ticks = virtual_clock.load(Ordering::Relaxed);
                            if let Err(e) = vcpu.vcpu.set_msrs(&[hypervisor::arch::x86::MsrEntry {
                                index: 0x10,
                                data: ticks,
                            }]) {
                                warn!("[det-sched] set MSR_IA32_TSC vcpu={vcpu_idx}: {e}");
                            }
                            ...
                            match vcpu.run() {
                                Ok(VmExit::IoapicEoi(vector)) => {
                                    ...
                                    // IoapicEoi counts as a real exit.
                                    real_exits += 1;
                                }
                                Ok(VmExit::Reset) => {
                                ...
                                Ok(VmExit::Ignore) => {
                                    // EINTR (preemption signal) or HLT/in-kernel
                                    // exit.  Count it as a real exit so the
                                    // scheduler rotates to the next vCPU.  A
                                    // halted AP will burn its entire quota on
                                    // Ignore exits and yield quickly, which is
                                    // exactly what we want: the BSP gets its
                                    // turn and can make progress.
                                    real_exits += 1;
                                }
                                Ok(_) => {
                                    // Debug, Hyperv, Tdx — count as real.
                                    real_exits += 1;
                                }
                                Err(e) => {
                                    error!("[det-sched] vcpu={vcpu_idx} error: {:?}", Error::VcpuRun(e.into()));
                                    for f in &interrupted_flags { f.store(true, Ordering::SeqCst); }
                                    drop(vcpu);
                                    exit_evt.write(1).unwrap();
                                    break 'outer;
                            ...
                            // One tick per VM exit (Ignore exits count too —
                            // they represent real guest work handled in-kernel).
                            virtual_clock.fetch_add(1, Ordering::Relaxed);
                        } // end slice
                    } // end per-vCPU round
                } // end 'outer
